...
layout(binding=4) uniform shadow_params {
    vec4 sh_vp[MAX_SHADOWED * MAX_SHADOW_VIEWS];   // mat4 as 4 rows each
    vec4 sh_rect[MAX_SHADOWED * MAX_SHADOW_VIEWS]; // atlas rect per cascade
...
void main() {
    ...
    // Alpha coverage decides whether this fragment exists, so resolve it
    // before normal mapping and clustered lighting work.
